...
LLM/providerの現行値は、まず最新日付の `research/official-model-delta-YYYY-MM-DD.html`（現在は `official-model-delta-2026-09-23.html`）と `workbench/knowledge/model-generations.html` を照合・更新します。トップ、coverage ledger、個別trackerからcurrent layerへリンクし、旧日付deltaはhistorical snapshotとして残します。X・日次ニュース・issue由来の情報は、公式sourceを確認するまで `RADAR_UNVERIFIED` とし、価格、期限、model ID、契約、securityの断定へ使いません。
